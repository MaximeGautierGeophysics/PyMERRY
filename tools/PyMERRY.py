...
class MERRY:
    ...
    Attributes:
        ...
        - cov_threshold: float > 0:
            Therehold value for not taking account of near zero coverage values
    ...
    def __init__(self, data, model, mesh,
        ...
        # Otional attributes: cov_threshold
        self.cov_threshold = cov_threshold

        # Check inputs and generate a quadrupole table:
        self._load_check_inputs()

        # Test if thee is topography in electrodes positions:
        pos = np.array(self.data.sensorPositions())
        pos_y, pos_z = pos[:, 1], pos[:, 2]
        if (pos_y != 0).all() or (pos_z != 0).all():  # topo
            print("A topography has been detected")
            print("DOI coeffs automatically set to 0.2 for all quadrupoles.")
            self.DOI_DD = 0.2
            self.DOI_WS = 0.2
            self.DOI_W = 0.2
            self.quads = self._generate_quadrupoles_table_topo()
        else:
    ...
    def _mid_point(ql, qr, x, y, DOI, **kwargs):
        slope = np.arctan((y[qr] - y[ql]) / (x[qr]-x[ql]))  # local slope
        # slope = kwargs.pop('slope', slope)  # use mean slope
        xd = x[ql] + (x[qr]-x[ql]) / 2
        yd = y[ql] + ((y[qr]-y[ql])/(x[qr]-x[ql])) * ((x[qr]-x[ql]) / 2)
        xdoi = xd - DOI * np.sin(-slope)
        ydoi = yd - DOI * np.cos(-slope)
        return xdoi, ydoi
...
###############################################################################
    ...
    def _generate_quadrupoles_table_topo(self) -> None:
        """
        electrodes to get L and use set the argument numerical=True for
        geometrical factor computing.
        Create a Panda DataFrame with information on each quadrupole.
        A line is a quadrupole with their index.

        Columns are:
            - potisions, xa, xb, xm, xn, ya, yb, ym, un, za, zb, zm, zn
            - index in the PyGimly DataContainerERT: idx_a, idx_b, idx_m, idx_n
            - apparent resistivity in ohm-mmeasured in the field: rhoa
            - standard deviation on each measurement: err
            - geometrical factor k in m, if not they are computed, without 2*pi
            - array: Wenner (W), Wenner-Schlumberger(WS), Dipole-Dipole (DD)
            - length of array L in m
            - coordinates of the depth of investigation: xDOI, yDOI
            - index of cell at the depth of investigation: idx_cell_DOI
        ...
        # Diatances and slopes between electrodes:
        dx = x[1:]-x[:-1]
        dy = y[1:]-y[:-1]
        dz = z[1:]-z[:-1]
        ...
        if np.all(k == 0):
            self.data['k'] = pg.physics.ert.createGeometricFactors(
                self.data, verbose=False, numerical=True)
        quads["k"] = self.data['k'] / (2*np.pi)
        self.k = np.array(quads["k"])
        ...
        if np.all(y == 0):  # No topography

            for i, q in enumerate(quads.itertuples()):

                # Wenner configuration:
                if abs(q.xm-q.xa) == abs(q.xn-q.xm) == abs(q.xb-q.xn):
                    quads.loc[i, "array"] = "W"
                    quads.loc[i, "L"] = abs(q.xa - q.xb)
                    quads.loc[i, "xDOI"] = (xd := q.xa + abs(q.xa - q.xb) / 2)
                    quads.loc[i, "yDOI"] = (
                        yd := - self.DOI_W*abs(q.xa - q.xb))
                    doi = (xd, yd)
                    quads.loc[i, "idx_cell_DOI"] = self.mesh.findCell(doi).id()

                # Wenner-Schlumberger configuration:
                elif ((q.xa < q.xm < q.xb) and (q.xa < q.xn < q.xb) and
                      abs(q.xm-q.xn) != abs(q.xa-q.xm)):
                    quads.loc[i, "array"] = "WS"
                    quads.loc[i, "L"] = abs(q.xa - q.xb)
                    quads.loc[i, "xDOI"] = (xd := q.xa + abs(q.xa - q.xb) / 2)
                    quads.loc[i, "yDOI"] = (
                        yd := - self.DOI_WS * abs(q.xa - q.xb))
                    doi = (xd, yd)
                    quads.loc[i, "idx_cell_DOI"] = self.mesh.findCell(doi).id()

                # Dipole-Dipole configuration:
                elif ((q.xa < q.xb < q.xm) and (q.xb < q.xm < q.xn) and
                      abs(q.xm-q.xa) != abs(q.xn-q.xm)):
                    quads.loc[i, "array"] = "DD"
                    quads.loc[i, "L"] = abs(q.xa - q.xb)
                    quads.loc[i, "xDOI"] = (xd := q.xa + abs(q.xa - q.xb) / 2)
                    quads.loc[i, "yDOI"] = (
                        yd := - self.DOI_DD * abs(q.xa - q.xb))
                    doi = (xd, yd)
                    quads.loc[i, "idx_cell_DOI"] = self.mesh.findCell(doi).id()

        elif not np.all(y == 0):  # Topography
            print("TOPO")
            for i, q in enumerate(quads.itertuples()):
                # Get a quadrupole:
                # q = quads.loc[i, :]

                # Get index of electrode A and B: TODO if W or WS or DD:
                # qa_idx = int(q['idx_a'])
                # qb_idx = int(q['idx_b'])
                # qm_idx = int(q['idx_m'])
                # qn_idx = int(q['idx_n'])
                ...
                if abs(q.xm-q.xa) == abs(q.xn-q.xm) == abs(q.xb-q.xn):
                    quads.loc[i, "array"] = "W"
                    ids = np.sort(np.array([q.idx_a, q.idx_b]))
                    L = np.sum(dist[ids[0]: ids[1]])
                    quads.loc[i, "L"] = L

                    DOI = self.DOI_W * L

                    mean_slope = np.mean(slopes[q.idx_a:q.idx_b])
                    xdoi, ydoi = MERRY._mid_point(q.idx_m, q.idx_n, x, y, DOI,
                                                  slope=mean_slope)
                    quads.loc[i, "xDOI"] = xdoi
                    quads.loc[i, "yDOI"] = ydoi
                    doi = (xdoi, ydoi)
                    quads.loc[i, "idx_cell_DOI"] = self.mesh.findCell(doi).id()
                ...
                # Dipole-Dipole configuration:
                      ...
                      abs(q.xm-q.xa) != abs(q.xn-q.xm)):
                    quads.loc[i, "array"] = "DD"
                    ids = np.sort(np.array([q.idx_a, q.idx_n]))
                    L = np.sum(dist[ids[0]: ids[1]])
                    quads.loc[i, "L"] = L

                    DOI = self.DOI_DD * L
                    mean_slope = np.mean(slopes[q.idx_a:q.idx_n])
                    xdoi, ydoi = MERRY._mid_point(q.idx_a, q.idx_n, x, y, DOI,
                                                  slope=mean_slope)
                ...
                # Unknown configuration:
                    ...
                    mean_slope = np.mean(slopes[ids[0]: ids[1]])
                    xdoi, ydoi = MERRY._mid_point(
                        q.idx_m, q.idx_n, x, y, DOI, slope=mean_slope)
                    ...
                    while check:
                        try:
                            quads.loc[i, "idx_cell_DOI"] = self.mesh.findCell(
                                doi).id()
                            check = False
                        except:
                            down = 1
                            ydoi -= down
                            print(f"quadrupole {i} ydoi -{down} m applied")
                            doi = (xdoi, ydoi)
    ...
    def create_masks(self, **kwargs) -> None:
        ...
        if self.coverage is None:
            self.coverage = MaskComputation.compute_coverage(
                self.masks, verbose=verbose, cov_threshold=self.cov_threshold)
...
class MaskComputation:
    ...
    def compute_coverage(masks: "np.ndarray", verbose=False,
                         **kwargs) -> "np.ndarray":
        """
        ...
        verbose : "bool", optional
            Write computational time is set to True. The default is False.
        cov_threshold : float > 0
            coverage threshold for not taking account of near zero coverage
            values.
        ...
        """
        cov_threshold = kwargs.pop("cov_threshold", None)
        to = time()
        ...
        # Add an "A posteriori" threshold on coverage for hight limit:
        if cov_threshold is not None:
            coverage[coverage <= cov_threshold] = 0
            coverage = coverage / np.max(coverage)
            print("  Coverage threshold applied.")
...
class InputTools:
    ...
    def load_parameters(file_name: str) -> dict:
        ...
        cells_path = InputTools.normalize_path(param_dict["cells_file"])
        nodes_path = InputTools.normalize_path(param_dict["nodes_file"])
        model_path = InputTools.normalize_path(param_dict["model_file"])
        data_path = InputTools.normalize_path(param_dict["data_file"])
    ...
    def save_parameters_file(inputs: dict) -> None:
        ...
        output_path = os.path.join(
            InputTools.normalize_path(inputs['save_dir']),
            "parameters_used" + '.txt')
